...
active_variant = gpu_config["active_model"]
model_path = gpu_config["model_variants"][active_variant]

print(f"\n[GPU Inference] Using model variant: {active_variant.upper()} -> {model_path}")
...
img = Image.open(gpu_config["image_path"])
...
save_emotion_json(predicted_emotion, gpu_config["output_json"])
...
# ------------------------------
img_cv = cv2.imread(gpu_config["image_path"])
img_cv = cv2.resize(img_cv, (960, 1280))

# Haarcascade Face Detection
face_cascade = cv2.CascadeClassifier(gpu_config["haar_cascade_path"])
...
cv2.imwrite(gpu_config["output_image"], img_cv)
print(f"[GPU Inference] Annotated image with timings saved to {gpu_config['output_image']}")
